...
Two gaps are documented rather than fixed. Neither loses a status change — a
board that is reloaded always shows the truth. Only the message about the change
is affected, which is why the cheap version shipped first.
...
and the local database never sees the write. No message is sent for a status
change made in cloud mode, from any client. The worker runs its own
