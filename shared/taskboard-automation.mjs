...
function buildTaskboardDeliveryLoopPrompt(request) {
  ...
  return [
    ...
    `Before any claim, run ${taskctlCommand} integration plan --project ${shellQuote(request.taskboardProjectId)} --json. Use the configured exact target branch; if absent, report the missing target policy and stop. The structured plan is canonical; integration-readiness:v1 cards are audit output only.`,
    "Consume decision.action and decision.claimIssueId. If decision.action is report, invalid, or no_action, report the plan and stop; never fall through to claiming todo. When decision.action is integrate, first route only the unique nextAction through its existing owner and separate authorization. A report-only recommendation never authorizes a merge. If decision.claimIssueId is also present, its disjoint lane may proceed independently; do not wait for unrelated integration work. Replan after any target change. Do not rebase the whole queue. No command here authorizes merge, release or done.",
    `If plan.nextAction.type is merge, the device-local exact-target authority policy, including explicit git-cas-push transport and allowForceWithLease consent, has authorized one integration. Run ${taskctlCommand} integration execute --project ${shellQuote(request.taskboardProjectId)} --target <plan.target.branch> --revision <plan.revision> --json once, passing values as quoted data arguments. The executor revalidates policy and inputs. Any stale, busy, ambiguous or execution_error result ends this integration attempt; do not retry or issue a direct provider merge. Replan after verified integration. This authority never includes release, production branches or done.`,
    "Only when decision.action is claim or integrate and decision.claimIssueId is non-null, inspect that exact decision.claimIssueId. Read the full issue, attachments and comments. Re-run integration plan immediately before the versioned claim; require the same revision, allowed issue and completed dependencies. Do not take over another active conversation. Planned write sets and domains are advisory; actual MR files replace them. Blocking findings return to the parent MR or stack behind it; non-blocking findings remain backlog until the parent integrates.",
    "Final delivery order: implement -> target sync/conflict resolution -> focused checks -> required runtime evidence -> push/MR -> integration plan. Preserve independent lanes and their exact-head evidence; refresh only conflicting or behaviorally affected branches.",
