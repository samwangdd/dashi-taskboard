...
6. Verify the requested operation path. Add a comment with the changes, verification result, outcome, and remaining risks. Read the issue again. If `issue get` returns `reviewRequired: true`, push the issue branch, create or reuse its PR/MR, and independently read back the remote SHA, PR/MR URL, source branch, and target branch. Local commits, tests, or local review cannot substitute for this published artifact. If publication is not authorized or cannot complete, keep the issue `in_progress` and record the exact gate. Otherwise, pass all five `--review-*` fields on the versioned move to `in_review`. An issue with `reviewRequired: false` may use the ordinary versioned move without review artifact fields.
