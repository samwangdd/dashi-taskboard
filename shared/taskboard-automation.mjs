...
// 发布证据是所有执行型自动化共享的状态门禁，不能只依赖某个下游 skill 的当前文案。
const REVIEW_ARTIFACT_INSTRUCTIONS = "当最新一次 issue get 返回 reviewRequired=true 时，本自动化明确授权执行开发的会话对该议题专属分支做普通 push，并创建或复用 PR/MR；该授权不授权 force push、merge、release 或 done。远程任务的首次指令也必须包含本条发布要求。发布后必须独立回读 remote SHA、PR/MR URL、source branch、target branch；本地 commit、测试或本地 review 不能替代已发布产物。只有五项证据齐全时，才可在带 version 与完整 binding 的 issue move --status in_review 中同时传 --review-provider <github|gitlab> --review-url <PR/MR URL> --review-remote-sha <remote SHA> --review-source-branch <source branch> --review-target-branch <target branch>。任一证据缺失、普通 push 失败或 PR/MR 无法创建/复用时，先评论准确 gate 并保持 in_progress，不得尝试无证据的 in_review 写入。";
...
export function buildTaskboardAutomationPrompt(request) {
  ...
  const executionInstructions = remoteProject
    ...
    : [
        ...
        REVIEW_ARTIFACT_INSTRUCTIONS,
        "执行完成并验证后，先用 comment add 记录关键改动、验证结果、执行结果和剩余风险。仅在上述发布契约满足后，才使用 ownedVersion、显式 --if-version、认领时保存的完整 binding 和五项 review artifact 证据将议题移动到 in_review；成功后更新 ownedVersion。不要省略 binding，避免把完整绑定降级为 legacy local；不要直接标记为 done。",
        `若本轮结束或暂停时，本轮认领或继续处理的议题仍停留在 in_progress，或本轮把议题移动到 blocked，必须先调用 $taskboard-handoff 生成交接卡，并追加一条评论，评论正文以 ${HANDOFF_CARD_MARKER} 开头，其中 Next action 需写明下一步要满足的具体 gate 或动作。若本轮是继续处理一个已绑定的 in_progress 议题，必须先读取最新一条以 ${HANDOFF_CARD_MARKER} 开头的评论，并从其 Next action 继续执行。`,
      ];
  return [
